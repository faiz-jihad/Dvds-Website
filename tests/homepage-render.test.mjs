      ...
      plugins: [
        {
          ...
          setup(b) {
            ...
            b.onResolve({ filter: /\/Spyker\w+$/ }, ({ path }) => ({
              path: path.split("/").at(-1),
              namespace: "spyker",
            }));
            b.onLoad({ filter: /.*/, namespace: "spyker" }, ({ path }) => ({
              contents: `import React from 'react'; export const ${path} = () => React.createElement('div', null, 'Starter ${path}');`,
            }));
            ...
            b.onLoad({ filter: /.*/, namespace: "section" }, ({ path }) => ({
              contents: `import React from 'react'; export const ${path} = ({data}) => React.createElement('section', null, data.title);`,
            }));
            ...
            b.onResolve(
              { filter: /\/(publicApi|homepageApi|Seo|AnimatedContent)$/ },
              ({ path }) => ({
                path: path.split("/").at(-1),
                namespace: "mock",
              }),
            ...
            b.onLoad({ filter: /.*/, namespace: "mock" }, ({ path }) => ({
              contents: {
                query:
                  "export const useQuery = ({queryKey}) => ({data: queryKey[0] === 'homepage' ? globalThis.config : []});",
                publicApi: "export const publicApi = {};",
                homepageApi: "export const homepageApi = {};",
                Seo: "export const Seo = () => null;",
                AnimatedContent:
                  "export const AnimatedContent = ({children}) => children;",
              }[path],
            }));
          },
        },
