          ...
          {faqs.map((faq, i) => (
            <div key={faq.q} className="py-4">
              <button
                ...
                className="w-full flex items-center justify-between text-left font-display font-bold text-sm sm:text-base text-dark hover:text-brand-blue transition-colors py-1"
              >
                <span>{faq.q}</span>
                <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${openIdx === i ? 'rotate-180 text-brand-blue' : 'text-gray-400'}`} />
              </button>
              {openIdx === i && (
                <div className="pt-2 text-xs sm:text-sm text-gray-600 leading-relaxed">
                  {faq.a}
                </div>
              )}
            </div>
