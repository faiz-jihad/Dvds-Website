    ...
    <div className="max-w-4xl mx-auto">
      ...
      {bankPending && <section className="bg-white rounded-2xl border border-blue-200 p-5 sm:p-7 mb-6">
        ...
        {bank?.bank_account_number ? <>
          <dl className="grid sm:grid-cols-2 gap-5 mt-6">
            {([['Bank', bank.bank_name], ['Account name', bank.bank_account_name], ['Sort code', bank.bank_sort_code], ['Account number', bank.bank_account_number], ['Amount to transfer', formatGBP(order.total_amount)], ['Payment reference', order.bank_transfer_reference || order.order_number]] as const).map(([label, value]) => <div key={label}><dt className="text-xs text-gray-500 mb-1">{label}</dt><dd className="flex items-center gap-2 text-sm font-semibold break-all">{value || '--'}{value && <button onClick={() => copy(value)} className="p-1 text-gray-400 hover:text-brand-blue" aria-label={`Copy ${label.toLowerCase()}`}><Copy size={14} /></button>}</dd></div>)}
          </dl>
      ...
      <div className="grid md:grid-cols-[minmax(0,1fr)_280px] gap-6">
        <section className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-7">
          ...
          <div className="divide-y divide-gray-100 my-5">{order.items.map((item) => <div key={item.id} className="flex gap-3 py-4"><div className="min-w-0 flex-1"><h3 className="font-medium text-sm">{item.product_title}</h3><p className="text-xs text-gray-500 mt-1">Qty {item.quantity} x {formatGBP(item.unit_price)}</p></div><span className="text-sm font-medium">{formatGBP(item.total_price)}</span></div>)}</div>
          <dl className="text-sm space-y-3 border-t border-gray-100 pt-5">
            <div className="flex justify-between"><dt className="text-gray-500">Subtotal</dt><dd>{formatGBP(order.subtotal)}</dd></div>
            {order.discount_amount > 0 && <div className="flex justify-between text-emerald-700"><dt>Discount</dt><dd>-{formatGBP(order.discount_amount)}</dd></div>}
            <div className="flex justify-between"><dt className="text-gray-500">Delivery</dt><dd>{order.shipping_amount === 0 ? 'FREE' : formatGBP(order.shipping_amount)}</dd></div>
            <div className="flex justify-between font-bold text-lg pt-3 border-t border-gray-100"><dt>Order total</dt><dd>{formatGBP(order.total_amount)}</dd></div>
            {Number(order.refunded_amount) > 0 && <div className="flex justify-between text-blue-700"><dt>Refunded</dt><dd>{formatGBP(order.refunded_amount!)}</dd></div>}
          </dl>
        </section>
        <aside className="space-y-5">
          ...
          <section className="bg-white rounded-2xl border border-gray-200 p-5"><h2 className="font-semibold text-sm">Delivery address</h2><address className="not-italic text-sm text-gray-600 leading-relaxed mt-3">{order.shipping_address.full_name}<br />{order.shipping_address.address_line_1}<br />{order.shipping_address.address_line_2 && <>{order.shipping_address.address_line_2}<br /></>}{order.shipping_address.city}, {order.shipping_address.postcode}<br />{order.shipping_address.country}</address></section>
